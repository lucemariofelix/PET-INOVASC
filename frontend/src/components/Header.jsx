      ...
      <div className="py-4 px-4 sm:px-8 flex items-center justify-between max-w-screen-2xl mx-auto">
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <nav className="flex flex-wrap justify-center gap-2 bg-sky-900/50 p-1 rounded-lg">
            <button onClick={() => handleTabClick('dashboard')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors cursor-pointer ${activeTab === 'dashboard' ? 'bg-white text-sky-800 shadow-sm' : 'text-sky-100 hover:text-white'}`}>
              <FaHeartbeat /> Alertas
            </button>
            <button onClick={() => handleTabClick('pacientes')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors cursor-pointer ${activeTab === 'pacientes' ? 'bg-white text-sky-800 shadow-sm' : 'text-sky-100 hover:text-white'}`}>
              <FaList /> Pacientes
            </button>
            <button onClick={() => handleTabClick('cadastro')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors cursor-pointer ${activeTab === 'cadastro' ? 'bg-white text-sky-800 shadow-sm' : 'text-sky-100 hover:text-white'}`}>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-sky-800 border-t border-sky-700 px-4 pt-3 pb-5 shadow-inner animate-in slide-in-from-top-2 duration-200 ease-out">
          <nav className="flex flex-col gap-2">
            <button onClick={() => handleTabClick('dashboard')} className={`flex items-center gap-3 w-full px-4 py-3 rounded-md text-base font-medium transition-colors ${activeTab === 'dashboard' ? 'bg-sky-100 text-sky-900' : 'text-sky-100 hover:bg-sky-700'}`}>
              <FaHeartbeat className={activeTab === 'dashboard' ? 'text-sky-700' : 'text-sky-300'} /> Alertas
            </button>
            <button onClick={() => handleTabClick('pacientes')} className={`flex items-center gap-3 w-full px-4 py-3 rounded-md text-base font-medium transition-colors ${activeTab === 'pacientes' ? 'bg-sky-100 text-sky-900' : 'text-sky-100 hover:bg-sky-700'}`}>
              <FaList className={activeTab === 'pacientes' ? 'text-sky-700' : 'text-sky-300'} /> Pacientes
            </button>
            <button onClick={() => handleTabClick('cadastro')} className={`flex items-center gap-3 w-full px-4 py-3 rounded-md text-base font-medium transition-colors ${activeTab === 'cadastro' ? 'bg-sky-100 text-sky-900' : 'text-sky-100 hover:bg-sky-700'}`}>
              <FaUserPlus className={activeTab === 'cadastro' ? 'text-sky-700' : 'text-sky-300'} /> Novo Paciente
            </button>
            <button onClick={() => handleTabClick('consulta')} className={`flex items-center gap-3 w-full px-4 py-3 rounded-md text-base font-medium transition-colors ${activeTab === 'consulta' ? 'bg-sky-100 text-sky-900' : 'text-sky-100 hover:bg-sky-700'}`}>
              <FaCalendarPlus className={activeTab === 'consulta' ? 'text-sky-700' : 'text-sky-300'} /> Agendar Consulta
            </button>
          </nav>
          ...
          {usuario && (
            <div className="mt-5 pt-5 border-t border-sky-700 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-white text-base font-bold leading-tight">{usuario.nome}</p>
                  <p className="text-sky-300 text-xs uppercase font-bold tracking-wider">{usuario.funcao}</p>
                </div>
              </div>
              <button 
                onClick={onLogout}
