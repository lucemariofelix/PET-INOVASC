...
import { FaBullhorn, FaUsers, FaPaperPlane, FaFilter, FaInfoCircle, FaSpinner } from "react-icons/fa";
...
export default function Notificacoes({ usuario }) {
  ...
  useEffect(() => {
    const carregarPacientes = async () => {
      try {
        ...
        setFeedback({ tipo: "erro", texto: "Não foi possível carregar a lista de pacientes." });
      } finally {
        setLoading(false);
      }
    };
  ...
  const pacientesFiltrados = useMemo(() => {
    return pacientes.filter(p => {
      ...
      const bateCondicao = filtroCondicao ? p.condicao === filtroCondicao : true;
  ...
  const handleDisparar = async () => {
    if (pacientesFiltrados.length === 0) {
      setFeedback({ tipo: "erro", texto: "Nenhum paciente selecionado para envio." });
      return;
    }
    if (!mensagem.includes("{nome}")) {
      setFeedback({ tipo: "erro", texto: "A mensagem precisa conter a variável {nome}." });
      return;
    }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-in fade-in duration-300">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Mensageria e Avisos</h2>
          <p className="text-gray-500 text-sm">Dispare mensagens em lote via WhatsApp com segurança anti-ban.</p>
        </div>
      </div>

      {feedback && (
        <div className={`p-4 rounded-md mb-6 font-medium ${feedback.tipo === "sucesso" ? "bg-green-100 text-green-800 border border-green-200" : "bg-red-100 text-red-800 border border-red-200"}`}>
          {feedback.texto}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white p-5 rounded-xl shadow-sm border border-gray-100 h-fit">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Agente de Saúde (ACS)</label>
              <select 
                value={filtroAcs} 
              ...
              >
                ...
                {listaAcs.map(acs => (
                  <option key={acs} value={acs}>{acs}</option>
                ))}
              </select>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1">Condição de Saúde</label>
              <select 
            ...
            <div className="mt-6 pt-4 border-t border-gray-100">
              <div className="bg-sky-50 p-4 rounded-lg flex flex-col items-center justify-center border border-sky-100">
                ...
                <span className="text-3xl font-bold text-sky-800">{loading ? "..." : pacientesFiltrados.length}</span>
                <span className="text-xs text-sky-600 font-medium uppercase tracking-wide text-center mt-1">Pacientes Selecionados</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="mb-4 bg-amber-50 border border-amber-200 p-3 rounded-md flex items-start gap-3">
            ...
            <p className="text-sm text-amber-800">
              Use a variável <strong className="bg-amber-200 px-1 rounded">{'{nome}'}</strong> no texto. O sistema substituirá automaticamente pelo primeiro nome do paciente para deixar a mensagem mais acolhedora.
            </p>
          </div>
